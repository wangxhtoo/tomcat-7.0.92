...
public interface ServletResponse {
	 ...
	 * explicitly using the {@link #setCharacterEncoding} or {@link #setContentType}
	 * methods, or implicitly using the {@link #setLocale} method. Explicit
	 * specifications take precedence over implicit specifications. Calls made to
	 * these methods after <code>getWriter</code> has been called or after the
	 * response has been committed have no effect on the character encoding. If no
	 * character encoding has been specified, <code>ISO-8859-1</code> is returned.
	 * <p>
	 * See RFC 2047 (http://www.ietf.org/rfc/rfc2047.txt) for more information about
	 * character encoding and MIME.
	 * 
	 * @return a <code>String</code> specifying the name of the character encoding,
	 ...
	 * encoding has been explicitly or implicitly specified as described in
	 * {@link #getCharacterEncoding}, the charset parameter is included in the
	 * string returned. If no character encoding has been specified, the charset
	 ...
	 * method or {@link #getWriter} may be called to write the body, not both.
	 * 
	 ...
	 * client. The <code>PrintWriter</code> uses the character encoding returned by
	 * {@link #getCharacterEncoding}. If the response's character encoding has not
	 * been specified as described in <code>getCharacterEncoding</code> (i.e., the
	 * method just returns the default value <code>ISO-8859-1</code>),
	 ...
	 *                                  character encoding returned by
	 *                                  <code>getCharacterEncoding</code> cannot be
	 *                                  used
	 * @exception IllegalStateException if the <code>getOutputStream</code> method
	 ...
	 * by {@link #setContentType} or {@link #setLocale}, this method overrides it.
	 * Calling {@link #setContentType} with the <code>String</code> of
	 * <code>text/html</code> and calling this method with the <code>String</code>
	 * of <code>UTF-8</code> is equivalent with calling <code>setContentType</code>
	 * with the <code>String</code> of <code>text/html; charset=UTF-8</code>.
	 * <p>
	 * This method can be called repeatedly to change the character encoding. This
	 * method has no effect if it is called after <code>getWriter</code> has been
	 ...
	 * <code>Content-Type</code> header for text media types. Note that the
	 ...
	 * 
	 * @param len an integer specifying the length of the content being returned to
	 ...
	 * sent, thus providing the servlet with more time to set appropriate status
	 * codes and headers. A smaller buffer decreases server memory load and allows
	 ...
	 * @exception IllegalStateException if this method is called after content has
	 *                                  been written
	 * @see #getBufferSize
	 * @see #flushBuffer
	 * @see #isCommitted
	 ...
	 * <code>getWriter</code> hasn't been called yet, and the response hasn't been
	 * committed yet. If the deployment descriptor contains a
	 * <code>locale-encoding-mapping-list</code> element, and that element provides
	 * a mapping for the given locale, that mapping is used. Otherwise, the mapping
	 * from locale to character encoding is container dependent.
	 * <p>
	 * This method may be called repeatedly to change locale and character encoding.
	 * The method has no effect if called after the response has been committed. It
	 * does not set the response's character encoding if it is called after
	 * {@link #setContentType} has been called with a charset specification, after
	 * {@link #setCharacterEncoding} has been called, after <code>getWriter</code>
	...
	/**
	 * Returns the locale specified for this response using the {@link #setLocale}
	 * method. Calls made to <code>setLocale</code> after the response is committed
	 * have no effect. If no locale has been specified, the container's default
	 * locale is returned.
	 * 
	 * @see #setLocale
	 */
