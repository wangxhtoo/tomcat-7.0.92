...
public abstract class HttpServlet extends GenericServlet {
	 ...
	 * an HTTP HEAD request. A HEAD request is a GET request that returns no body in
	 * the response, only the request header fields.
	 *
	 * <p>
	 ...
	 * write the response data. It's best to include content type and encoding. When
	 ...
	 * {@link javax.servlet.ServletResponse#setContentLength} method), to allow the
	 * servlet container to use a persistent connection to return its response to
	 * the client, improving performance. The content length is automatically set if
	 ...
	 *
	 * <p>
	 * If the request is incorrectly formatted, <code>doGet</code> returns an HTTP
	 * "Bad Request" message.
	 *
	 ...
	 * modified, in milliseconds since midnight January 1, 1970 GMT. If the time is
	 ...
	 *         milliseconds since midnight, January 1, 1970 GMT, or -1 if the time
	 *         is not known
	 */
	...
	/**
	 * <p>
	 * Receives an HTTP HEAD request from the protected <code>service</code> method
	 * and handles the request. The client sends a HEAD request when it wants to see
	 * only the headers of a response, such as Content-Type or Content-Length. The
	 * HTTP HEAD method counts the output bytes in the response to set the
	 ...
	 * the <code>doHead</code> method you write is both safe and idempotent (that
	 ...
	 *
	 * @param resp the response object that the servlet uses to return the headers
	 *             to the client
	 *
	 ...
	 * the Web server a single time and is useful when posting information such as
	 * credit card numbers.
	 *
	 * <p>
	 * When overriding this method, read the request data, write the response
	 ...
	 * <code>doPut</code> performs can have side effects for which the user can be
	 * held accountable. When using this method, it may be useful to save a copy of
	 * the affected URL in temporary storage.
	 *
	 * <p>
	 * If the HTTP PUT request is incorrectly formatted, <code>doPut</code> returns
	 ...
	 *
	 * The DELETE operation allows a client to remove a document or Web page from
	 * the server.
	 *
	 * <p>
	 * This method does not need to be either safe or idempotent. Operations
	...
	/**
	 * Receives standard HTTP requests from the public <code>service</code> method
	 * and dispatches them to the <code>do</code><i>Method</i> methods defined in
