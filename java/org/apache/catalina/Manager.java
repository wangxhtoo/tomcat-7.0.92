...
public interface Manager {
	...
	/**
	 * Set the Container with which this Manager is associated. The Container must
	 * be set to a non-null value before the Manager is first used. Multiple calls
	 * to this method before first use are permitted. Once the Manager has been
	 * used, this method may not be used to change the Container (including setting
	 * a {@code null} value) that the Manager is associated with.
	 *
	...
	/**
	 * Set the default maximum inactive interval (in seconds) for Sessions created
	 * by this Manager.
	 *
	 ...
	 * time.
	 *
	 * @param maxActive Maximum number of sessions that have been active at the same
	 *                  time.
	 */
	...
	/**
	 * Gets the number of sessions that were not created because the maximum number
	 * of active sessions was reached.
	 *
	 ...
	 *
	 * @param sessionMaxAliveTime Longest time (in seconds) that an expired session
	 *                            had been alive.
	 */
	public void setSessionMaxAliveTime(int sessionMaxAliveTime);

	/**
	 * Gets the average time (in seconds) that expired sessions had been alive. This
	...
	/**
	 * Gets the current rate of session creation (in session per minute). This may
	 * be based on sample data.
	 *
	...
	/**
	 * Gets the current rate of session expiration (in session per minute). This may
	 * be based on sample data
	 *
	 ...
	 * specified by this Manager's properties. The session id specified will be used
	 * as the session id. If a new session cannot be created for any reason, return
	 * <code>null</code>.
	 *
	 * @param sessionId The session id which should be used to create the new
	 *                  session; if <code>null</code>, the session id will be
	 *                  assigned by this method, and available via the getId()
	 *                  method of the returned session.
	 * @exception IllegalStateException if a new session cannot be instantiated for
	 *                                  any reason
	 */
	 ...
	 * appropriate persistence mechanism, if any. If persistence is not supported,
	 * this method returns without doing anything.
	 *
	 * @exception ClassNotFoundException if a serialized class cannot be found
	 *                                   during the reload
	...
	/**
	 * Save any currently active sessions in the appropriate persistence mechanism,
	 * if any. If persistence is not supported, this method returns without doing
	 * anything.
	 *
	 * @exception IOException if an input/output error occurs
	 */
	...
	/**
	 * This method will be invoked by the context/container on a periodic basis and
	 * allows the manager to implement a method that executes periodic tasks, such
	 * as expiring sessions etc.
	 */
	public void backgroundProcess();

	/**
	 * Would the Manager distribute the given session attribute? Manager
	 * implementations may provide additional configuration options to control which
	 * attributes are distributable.
	 *
