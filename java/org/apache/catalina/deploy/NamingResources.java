...
public class NamingResources extends LifecycleMBeanBase implements Serializable {
	...
	/**
	 * Return the defined message destination references for this application. If
	 * there are none, a zero-length array is returned.
	 */
	...
	/**
	 * Return the web service reference for the specified name, if any; otherwise
	 * return <code>null</code>.
	 *
	...
	/**
	 * Return the defined web service references for this application. If none have
	 * been defined, a zero-length array is returned.
	 */
	...
	/**
	 * Clean up a resource by calling the defined close method. For example, closing
	 * a database connection pool will close it's open connections. This will happen
	 * on GC but that leaves db connections open that may cause issues.
	 * 
	 ...
	 * consistent with any injection targets and if the type is not specified, tries
	 * to configure the type based on the injection targets
	 *
	 ...
	 *         <code>false</code> if the current resource type is inconsistent with
	 *         the injection targets and/or cannot be determined
	 */
