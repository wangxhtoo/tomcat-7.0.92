...
public class Catalina {
	...
	/**
	 * Return a File object representing our configuration file. </br>
	 * 返回server.xml文件
	 */
	...
	protected Digester createStartDigester() {
		...
		// 连接器规则创建类，根据server.xml文件中配置的协议进行初始化Connector，默认Http11Protocol
		digester.addRule("Server/Service/Connector", new ConnectorCreateRule());
		digester.addRule("Server/Service/Connector", new SetAllPropertiesRule(new String[] { "executor" }));
		digester.addSetNext("Server/Service/Connector", "addConnector", "org.apache.catalina.connector.Connector");
		// 指定Connector下的Listener
		// 第二个参数null必须指定 MUST be specified in the element
	...
	public void load() {
		...
		// Create and execute our Digester
		// 初始化Digester结构，即解析server.xml文件的结构
		...
		try {
			// StandardServer，标准server组件
			// 服务组件初始化。调用LifecycleBase的init方法实现类，并最终调用StandardServer的initInternal方法
			getServer().init();
		} catch (LifecycleException e) {
	...
	/**
	 * Start a new server instance. </br>
	 * 开始一个新的server实例
	 */
	public void start() {
		// 如果server为空，则先初始化server
		if (getServer() == null) {
			load();
		}
		// 初始化后还失败，则停止执行
		...
		// Start the new server
		try {
			// 调用StandardServer的start方法，启动server
			// 即调用了LifecycleBase的start方法后，再调用StandardServer的initInternal方法
			getServer().start();
		} catch (LifecycleException e) {
