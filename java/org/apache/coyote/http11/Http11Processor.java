 ...
 * 
 * 
 * 提供了对HTTP协议通信的处理，包括对套接字的读写和过滤，对HTTP协议的解析以及封装成请求对象，HTTP协议相应对象的生成等
 * 
 *
...
public class Http11Processor extends AbstractHttp11Processor<Socket> {
	...
	public Http11Processor(int headerBufferSize, boolean rejectIllegalHeaderName, JIoEndpoint endpoint,
	...
	protected void setRequestLineReadTimeout() throws IOException {
		...
		/*
		 * When there is no data in the buffer and this is not the first request on this
		 * connection and timeouts are being used the first read for this request may
		 * need a different timeout to take account of time spent waiting for a
		 * processing thread.
		 * 
		 * This is a little hacky but better than exposing the socket and the timeout
		 * info to the InputBuffer
		 */
	...
	public void actionInternal(ActionCode actionCode, Object param) {
		...
		case REQ_SSL_CERTIFICATE: {
			if (sslSupport != null) {
				/*
				 * Consume and buffer the request body, so that it does not interfere with the
