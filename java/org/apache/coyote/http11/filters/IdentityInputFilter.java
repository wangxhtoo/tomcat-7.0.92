 ...
 * 
 * 
 * 在HTTP包含content-length头部并且指定的长度大于0时使用，它将根据指定的长度从底层读取响应长度的字节数组，当读取足够数据后，将直接返回-1，避免再次执行底层操作
 * 
 * 
...
public class IdentityInputFilter implements InputFilter {
	 ...
	 * Read bytes.
	 * 
	 * @return If the filter does request length control, this value is significant;
	 *         it should be the number of bytes consumed from the buffer, up until
	 *         the end of the current request body, or the buffer length, whichever
	 *         is greater. If the filter does not do request body length control,
