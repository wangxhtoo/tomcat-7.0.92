 ...
 * 
 * 处理逻辑：</br>
 * 1、判断是不是第一次访问JSP，如果是，则会先编译JSP页面，按一定包和类命名规则生成对应的Servlet类。</br>
 * 2、加载刚编译好的JSP Servlet 类，并初始化他们。</br>
 * 3、调用刚刚加载好的JSP Servlet的service方法，处理请求。</br>
 * 
 *
...
public class JspServlet extends HttpServlet implements PeriodicEventListener {
	...
	// jspFile for a jsp configured explicitly as a servlet, in environments where
	// this
	 ...
	 *
	 * @return The number of JSPs that have been loaded into the webapp with which
	...
	/**
	 * <p>
	 * Look for a <em>precompilation request</em> as described in Section 8.4.2 of
	 * the JSP 1.2 Specification. <strong>WARNING</strong> - we cannot use
	 * <code>request.getParameter()</code> for this, because that will trigger
	 * parsing all of the request parameters, and not give a servlet the opportunity
	 * to call <code>request.setCharacterEncoding()</code> first.
	 * </p>
	 *
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (jspUri == null) {
			...
			if (jspUri != null) {
				/*
				 * Requested JSP has been target of RequestDispatcher.include(). Its path is
				 * assembled from the relevant javax.servlet.include.* request attributes
				 */
				...
				/*
				 * Requested JSP has not been the target of a RequestDispatcher.include().
				 * Reconstruct its path from the request's getServletPath() and getPathInfo()
				 */
