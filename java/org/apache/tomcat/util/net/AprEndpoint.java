...
public class AprEndpoint extends AbstractEndpoint<Long> {
		...
		if (SSL_PROTO_ALL == null) {
			synchronized (SSL_PROTO_ALL_LOCK) {
				if (SSL_PROTO_ALL == null) {
					...
					/*
					 * Default used if SSLProtocol is not configured, also used if SSLProtocol="All"
					 */
	...
	/**
	 * SSL password (if a cert is encrypted, and no password has been provided, a
	 * callback will ask for a password).
	 */
	...
	/**
	 * SSL allow insecure renegotiation for the the client that does not support the
	...
	/**
	 * Set to <code>true</code> to enforce the <i>server's</i> cipher order instead
	 * of the default which is to allow the client to choose a preferred cipher.
	 */
	 ...
	 * possibly improves performance by not compressing uncompressible content such
	 * as JPEG, etc.
	 */
