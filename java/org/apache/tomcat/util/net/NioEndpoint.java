...
public class NioEndpoint extends AbstractEndpoint<NioChannel> {
	...
	/**
	 * The oom parachute, when an OOM error happens, will release the data, giving
			...
			try {
				for (Iterator<SelectionKey> iter = keys.iterator(); iter.hasNext();) {
					...
					try {
						...
						if (ka == null) {
							cancelledKey(key, SocketStatus.ERROR, false); // we don't support any keys without
																			// attachments
						} else if (ka.getError()) {
							...
							// if (!processSocket(ka.getChannel(), SocketStatus.OPEN_CALLBACK))
...
// ----------------------------------------------------- Key Attachment Class
	public static class KeyAttachment extends SocketWrapper<NioChannel> {
		...
		/**
		 * @deprecated Unused (value is set but never read) - will be removed in Tomcat
		 *             8
		 */
		@Deprecated
