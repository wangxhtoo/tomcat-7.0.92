...
public class SocketWrapper<E> {
	...
	/*
	 * In normal servlet processing only one thread is allowed to access the socket
	 * at a time. That is controlled by a lock on the socket for both read and
	 * writes). When HTTP upgrade is used, one read thread and one write thread are
	 * allowed to access the socket concurrently. In this case the lock on the
	 * socket is used for reads and the lock below is used for writes.
	 */
